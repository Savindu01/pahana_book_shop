...
function Customer() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(`Failed to ${currentCustomer ? 'update' : 'save'} customer`);
      }
    } catch (error) {
      console.error(`Error ${currentCustomer ? 'updating' : 'saving'} customer:`, error);
      Swal.fire({
        icon: "error",
        title: "Error",
        text: `Failed to ${currentCustomer ? 'update' : 'save'} customer. Please try again.`,
      });
    }
  };
  ...
  const deleteCustomer = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(
          `http://localhost:8080/api/v1/customer/delete-customer/${id}`,
          {
            method: "DELETE",
          }
        );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 ">
          ...
          <p className="text-gray-400 mt-2">
            Manage your bookstore customers with ease
          </p>
        </div>
